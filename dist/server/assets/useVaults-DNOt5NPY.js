...
import { I as useWallet, h as PublicKey, e as LayoutExports, s as getReadOnlyProgram, m as connection, P as PROGRAM_ID, v as getVaultTokenPDA, q as getPlatformConfigPDA } from "./router-B3y7OxSE.js";
import require$$1 from "fs";
import require$$0 from "path";
function useAnchorWallet() {
  const { publicKey: publicKey2, signTransaction, signAllTransactions } = useWallet();
  return reactExports.useMemo(() => publicKey2 && signTransaction && signAllTransactions ? { publicKey: publicKey2, signTransaction, signAllTransactions } : void 0, [publicKey2, signTransaction, signAllTransactions]);
}
...
function requireBindings() {
  ...
  (function(module, exports) {
    var fs = require$$1, path = require$$0, fileURLToPath = requireFileUriToPath(), join = path.join, dirname = path.dirname, exists = fs.accessSync && function(path2) {
      try {
        fs.accessSync(path2);
      } catch (e) {
        return false;
...
const bigInt = (length) => (property) => {
  ...
  bigIntLayout.decode = (buffer, offset) => {
    ...
    return nodeExports.toBigIntLE(Buffer.from(src));
  };
  bigIntLayout.encode = (bigInt2, buffer, offset) => {
    const src = nodeExports.toBufferLE(bigInt2, length);
...
const publicKey = (property) => {
  const layout = LayoutExports.blob(32, property);
  const { encode, decode } = encodeDecode(layout);
...
const MultisigLayout = LayoutExports.struct([
  LayoutExports.u8("m"),
  LayoutExports.u8("n"),
  bool("isInitialized"),
  publicKey("signer1"),
...
const AccountLayout = LayoutExports.struct([
  publicKey("mint"),
  publicKey("owner"),
  u64("amount"),
  LayoutExports.u32("delegateOption"),
  publicKey("delegate"),
  LayoutExports.u8("state"),
  LayoutExports.u32("isNativeOption"),
  u64("isNative"),
  u64("delegatedAmount"),
  LayoutExports.u32("closeAuthorityOption"),
...
const MintLayout = LayoutExports.struct([
  LayoutExports.u32("mintAuthorityOption"),
...
function identifyRegistrationError({ error, options }) {
  var _a, _b, _c;
  const { publicKey: publicKey2 } = options;
  if (!publicKey2) {
  ...
  if (error.name === "AbortError") {
    ...
    if (((_a = publicKey2.authenticatorSelection) === null || _a === void 0 ? void 0 : _a.requireResidentKey) === true) {
      return new WebAuthnError({
      ...
      options.mediation === "conditional" && ((_b = publicKey2.authenticatorSelection) === null || _b === void 0 ? void 0 : _b.userVerification) === "required"
    ) {
      return new WebAuthnError({
    ...
    } else if (((_c = publicKey2.authenticatorSelection) === null || _c === void 0 ? void 0 : _c.userVerification) === "required") {
      return new WebAuthnError({
    ...
    const validPubKeyCredParams = publicKey2.pubKeyCredParams.filter((param) => param.type === "public-key");
    ...
    } else if (publicKey2.rp.id !== effectiveDomain) {
      return new WebAuthnError({
        message: `The RP ID "${publicKey2.rp.id}" is invalid for this domain`,
        code: "ERROR_INVALID_RP_ID",
        cause: error
      });
    }
  } else if (error.name === "TypeError") {
    if (publicKey2.user.id.byteLength < 1 || publicKey2.user.id.byteLength > 64) {
      return new WebAuthnError({
...
function identifyAuthenticationError({ error, options }) {
  ...
  if (error.name === "AbortError") {
    ...
    } else if (publicKey2.rpId !== effectiveDomain) {
      return new WebAuthnError({
        message: `The RP ID "${publicKey2.rpId}" is invalid for this domain`,
        code: "ERROR_INVALID_RP_ID",
        cause: error
      });
    }
...
class GoTrueClient {
  ...
  async getClaims(jwt, options = {}) {
    try {
      ...
      const publicKey2 = await crypto.subtle.importKey("jwk", signingKey, algorithm, true, [
        "verify"
      ]);
      const isValid = await crypto.subtle.verify(algorithm, publicKey2, signature, stringToUint8Array(`${rawHeader}.${rawPayload}`));
      if (!isValid) {
...
const supabaseUrl = "https://rumjcnykuaaywifexppi.supabase.co";
const supabaseAnonKey = "sb_publishable_giA3udaXLEnywejB_V6lew_Y21wbKNJ";
const isRealSupabaseConfigured = Boolean(supabaseAnonKey);
const supabase = isRealSupabaseConfigured ? createClient(supabaseUrl, supabaseAnonKey) : null;
...
function getLocalStorageVaults() {
...
function saveLocalStorageVault(vault) {
  if (typeof window === "undefined") return;
  try {
    const vaults = getLocalStorageVaults();
    vaults[vault.id] = {
      ...vault,
      created_at: vault.created_at || (/* @__PURE__ */ new Date()).toISOString()
    };
...
async function saveVaultMetadata(data) {
  const record = {
    ...data,
...
async function getVaultMetadata(id) {
  if (supabase) {
    try {
...
async function getAllVaultsMetadata() {
  if (supabase) {
    try {
...
function getLocalStorageProfile(walletAddress) {
...
function saveLocalStorageProfile(profile) {
  if (typeof window === "undefined") return;
  try {
...
async function getUserProfile(walletAddress) {
  if (supabase) {
    try {
      const { data, error } = await supabase.from("profiles").select("*").eq("wallet_address", walletAddress).single();
      if (!error && data) {
        return data;
      }
    } catch (e) {
...
async function uploadAvatar(file, walletAddress) {
  if (supabase) {
    try {
...
function saveLocalStorageSwap(swap) {
  if (typeof window === "undefined") return;
  try {
    const swaps = getLocalStorageSwaps();
    swaps[swap.id] = {
      ...swap,
      timestamp: swap.timestamp || (/* @__PURE__ */ new Date()).toISOString()
    };
...
async function saveSwapEvent(data) {
  const record = {
    ...data,
...
async function getVaultSwaps(vaultPubkey) {
  if (supabase) {
    try {
      const { data, error } = await supabase.from("swaps").select("*").eq("vault_pubkey", vaultPubkey).order("timestamp", { ascending: false });
      if (!error && data) {
        return data;
      }
    } catch (e) {
  ...
  const swaps = getLocalStorageSwaps();
  return Object.values(swaps).filter((s) => s.vault_pubkey === vaultPubkey).sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime());
}
async function getUserSwaps(userWalletAddress) {
  if (supabase) {
    try {
      const { data, error } = await supabase.from("swaps").select("*").eq("user_wallet", userWalletAddress).order("timestamp", { ascending: false });
      if (!error && data) {
        return data;
      }
    } catch (e) {
  ...
  const swaps = getLocalStorageSwaps();
  return Object.values(swaps).filter((s) => s.user_wallet === userWalletAddress).sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime());
}
async function getGlobalSwaps(limit = 12) {
  if (supabase) {
    try {
      const { data, error } = await supabase.from("swaps").select("*").order("timestamp", { ascending: false }).limit(limit);
      if (!error && data) {
        return data;
      }
    } catch (e) {
  ...
  const swaps = getLocalStorageSwaps();
  return Object.values(swaps).sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime()).slice(0, limit);
}
...
async function saveVaultComment(data) {
...
async function getVaultComments(vaultId) {
  if (supabase) {
    try {
      const { data, error } = await supabase.from("comments").select("*").eq("vault_id", vaultId).order("created_at", { ascending: false });
      if (!error && data) {
        return data;
      }
    } catch (e) {
  ...
  const comments = getLocalStorageComments();
  return Object.values(comments).filter((c) => c.vault_id === vaultId).sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
}
async function fetchVaultTokens(vaultPubkey, tokenMints, offChainTokens) {
  const tokens = [];
  for (const mint of tokenMints) {
    ...
    const offChainToken = offChainTokens.find((t) => t.mint === mintStr);
...
function useVaults() {
  ...
  const [onChainVaults, setOnChainVaults] = reactExports.useState([]);
  const fetchVaults = reactExports.useCallback(async () => {
    setLoading(true);
    try {
      ...
      for (const rawVault of allVaults) {
        ...
        const lockType = "permanent" in lockTypeValue ? "permanent" : "timed";
        chainVaults.push({
          ...
          token_mints: account.tokenMints.map((m) => m.toBase58()),
...
function useVaultDetail(id) {
  const wallet = useAnchorWallet();
  const [vault, setVault] = reactExports.useState(null);
  ...
  const addLocalSwap = reactExports.useCallback((amount, tokenInMint, tokenOutMint) => {
    if (!vault) return;
        ...
        try {
          ...
          for (const revAcc of revAccounts) {
            ...
            revShares.push({
              ...
              added_at: addedAtTs > 0 ? new Date(addedAtTs * 1e3).toISOString() : (/* @__PURE__ */ new Date()).toISOString()
            });
          }
        } catch (err) {
...
const DEFAULT_PLATFORM_CONFIG = {
  ...
  creatorFee: 2e-3,
  // 2_000_000 lamports
  minimumDepositBps: 100
  // 1%
};
function usePlatformConfig() {
  const [config, setConfig] = reactExports.useState(null);
  const [loading, setLoading] = reactExports.useState(true);
  const [initialized, setInitialized] = reactExports.useState(true);
  reactExports.useEffect(() => {
    async function fetchConfig() {
      try {
        ...
        setConfig({
          ...
          creationFee: Number(rawConfig.creationFee) / 1e9,
          swapFee: Number(rawConfig.swapFee) / 1e9,
          platformFee: Number(rawConfig.platformFee) / 1e9,
          creatorFee: Number(rawConfig.creatorFee) / 1e9,
  ...
  reactExports.useEffect(() => {
    async function fetchStats() {
      try {
        ...
        const allVaults = await program.account.vault.all();
        const creators = new Set(allVaults.map((v) => v.account.creator.toBase58()));
        ...
        setStats({
          ...
          total_swaps: 0,
          total_sol_fees: platformWalletBalance / 1e9,
...
function useGlobalSwapFeed(limit = 12) {
  const [swaps, setSwaps] = reactExports.useState([]);
  reactExports.useEffect(() => {
...
export {
  ASSOCIATED_TOKEN_PROGRAM_ID as A,
  TOKEN_PROGRAM_ID as T,
  getAssociatedTokenAddress as a,
  getMint as b,
  getUserProfile as c,
  getUserSwaps as d,
  getVaultComments as e,
  saveUserProfile as f,
  getAccount as g,
  saveVaultComment as h,
  saveVaultMetadata as i,
  uploadToIPFS as j,
  useAnchorWallet as k,
  useGlobalSwapFeed as l,
  usePlatformConfig as m,
  usePlatformStats as n,
  useVaultDetail as o,
  useVaults as p,
  saveSwapEvent as s,
  uploadAvatar as u
